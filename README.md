...
解析站会对被风控的出口 IP（机房/云服务器常见）弹出**算术图片验证码**：页面跳到
`/toCaptcha/<卡密>`，图片是 `数字 运算符 数字 = ?`（130×48 彩色扭曲字体）。

装上 `ddddocr` 后会自动识别并通过：
...
- 验证是**卡密级**的，不是 session 级 —— 通过一次后，另起一个全新会话也能直接解析；
- 答错**没有惩罚**，只是重新出一张图，可以无限重试；
- 识别率约 80%，且识别不出时**返回空串弃权**（换一张重试）而不会提交错误答案；
  配合最多 4 次重试，全败概率约 0.2%。
- 成功判据是 `POST /doCaptcha` 返回 302，失败是 200 + 重新渲染验证页。

没装 ddddocr 时行为不变：返回 `code=403`，并打印验证链接供人工打开处理。

回归测试（需要 ddddocr，样本是人眼标注的真实验证码）：

```commandline
python tests/test_captcha_ocr.py
```
